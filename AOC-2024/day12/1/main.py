...
u = UnionFind(
        lambda a, b: {
            ...
            # we are only joining same-colored regions
            'color': a['color']
        }
    )
...
for y in range(h):
    for x in range(w):
        ...
        if y > 0 and map[y-1][x] == c: # will join up
            perimeter -= 2
        u.register((x, y), { 'area': 1, 'perimeter': perimeter, 'color': c })
...
for item, (parent, data) in u.unions.items():
    if parent is None:
        ...
        cost += c
        print(f'color: {data["color"]}\tarea: {data["area"]:5}\tperimeter: {data["perimeter"]:5}\tcost: {c:5}')
